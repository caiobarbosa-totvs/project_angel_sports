...
function validateCustomField(){
	...
	if(CURRENT_STATE == INICIO_0 || CURRENT_STATE == INICIO){
		...
		if($("input[name='rdClienteNovo']:checked").val() == undefined){
			errorMsg += "Campo 'Cliente novo?' é obrigatório!"+ lineBreaker;
		}
...
function validateField(el, selector){
	...
	if($("input[name='rdClienteNovo']:checked").val() === "nao") {
		...
		var camposCondicionais = ["catCliente", "numeroRg", "numPassaporte", "bairro", "numero", "email"];
		if (camposCondicionais.includes(campo)) {
			validation.status = "ignore";
			return validation;
		}
	}
